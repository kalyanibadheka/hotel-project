...
Class Action {
	...
	function save_category(){
		...
		if($_FILES['pic']['tmp_name'] != ''){
						$fname = strtotime(date('y-m-d H:i')).'_'.$_FILES['pic']['type'];
						$move = move_uploaded_file($_FILES['pic']['tmp_name'],'upload/'. $fname);
					$data .= ", cover_img = '$fname' ";

		}
		if(empty($id)){
			$save = $this->db->query("INSERT INTO registration set ".$data);
		}else{
			$save = $this->db->query("UPDATE registration set ".$data." where id=".$id);
		}
		if($save)
			return 1;
	}
	function delete_category(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM registration where id = ".$id);
		if($delete)
			return 1;
	}
	function save_room(){
